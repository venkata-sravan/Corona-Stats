...
def getMap(start,stop):
    ...
    for j in range(start, stop):
        ...
        T = tr_elements[j]
        try:
            if ((T[0].text_content().replace('\n', "").strip() not in z) and (int(T[4].text_content().replace(',', ""))!=0) and (int(T[1].text_content().replace(',', ""))>=1000)):
                New_Cases_Country.append(T[0].text_content())
                New_Cases.append(int(T[4].text_content().replace(',', "")))
        except:
            pass
...
try:
    with open('new_death_rate.csv', 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(['Rank','Country','Daily Deaths', 'Change in Death Rate'])
        i=0
...
new_deaths=pd.read_csv('new_death_rate.csv',index_col=[0,1,2,3],encoding = "ISO-8859-1")
new_deaths.to_html('change/daily_death_rate.html')
